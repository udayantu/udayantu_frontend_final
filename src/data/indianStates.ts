...
// Transform data into structured format with districts and blocks
export const indianStatesData = Object.keys(indianStatesDistricts).map((state) => {
  return {
    state,
    districts: indianStatesDistricts[state].map((district) => ({
      name: district,
      blocks: [
        `${district} Block 1`,
        `${district} Block 2`,
...
// Blocks/Cities by District (simplified structure with major blocks)
export const districtBlocks: Record<string, string[]> = {
  // This is a simplified structure. In production, you'd need complete data
  // Default blocks that will be used for districts not explicitly listed
};

// Function to get blocks for a district
export const getBlocksForDistrict = (district: string): string[] => {
  // Return specific blocks if available, otherwise return a placeholder
  return districtBlocks[district] || [
    `${district} Block 1`,
    `${district} Block 2`, 
    `${district} Block 3`,
    `${district} City`,
